...
export default function ForgotPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await forgotPassword(email); // ✅ Đổi authService.forgotPassword → forgotPassword
    ...
    return (
      <div className="w-full max-w-[400px] bg-white p-8 rounded-3xl shadow-lg border border-slate-100">
        ...
        <button
          ...
          className="mt-4 text-xs text-slate-500 hover:text-cyan-600 w-full text-center"
        >
          ← Quay lại nhập email
        </button>
      </div>
  ...
  return (
    <div>
      ...
      <button
        onClick={() => setStep('otp')}
        className="mt-4 text-xs text-slate-500 hover:text-cyan-600 w-full text-center block"
      >
        ← Quay lại nhập OTP
      </button>
      <Link
        to="/login"
        className="mt-2 text-xs text-cyan-600 hover:text-cyan-700 w-full text-center block font-semibold"
      >
        Đi đến đăng nhập →
      </Link>
    </div>
